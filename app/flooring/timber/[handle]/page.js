...
import { NATURAL_TIMBER_PRODUCTS, byHandle } from '@/components/naturalTimberProductData'
...
export function generateStaticParams() {
  return NATURAL_TIMBER_PRODUCTS.map((p) => ({ handle: p.handle }))
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    alternates: { canonical: `/flooring/timber/${p.handle}` },
  }
}
...
function relatedFor(handle) {
  const i = NATURAL_TIMBER_PRODUCTS.findIndex((p) => p.handle === handle)
  const out = []
  for (let k = 1; out.length < 4 && k < NATURAL_TIMBER_PRODUCTS.length; k++) {
    const p = NATURAL_TIMBER_PRODUCTS[(i + k) % NATURAL_TIMBER_PRODUCTS.length]
    if (p.handle !== handle) {
...
export default async function NaturalTimberProductPage({ params }) {
  const { handle } = await params
