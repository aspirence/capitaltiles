...
import { FLOORING_PRODUCTS, byHandle } from '@/components/flooringProductData'

export function generateStaticParams() {
  return FLOORING_PRODUCTS.map((p) => ({ handle: p.handle }))
}

/* Every board is also reachable under its collection — /flooring/timber/oak —
...
export async function generateMetadata({ params }) {
  ...
  /* Not a product: this route sits in front of the catch-all, so it has to
     answer for every /flooring/<something> address the site links to. It answers
...
export default async function FlooringProductPage({ params }) {
  const { handle } = await params
  const entry = byHandle(handle)
  if (!entry) return <ComingSoon slug={['flooring', handle]} />
