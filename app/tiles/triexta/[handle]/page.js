...
import { TRIEXTA_PRODUCTS, byHandle } from '@/components/triextaProductData'
...
export function generateStaticParams() {
  return TRIEXTA_PRODUCTS.map((p) => ({ handle: p.handle }))
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    alternates: { canonical: `/tiles/triexta/${p.handle}` },
  }
}
...
function relatedFor(handle) {
  const i = TRIEXTA_PRODUCTS.findIndex((p) => p.handle === handle)
  const out = []
  for (let k = 1; out.length < 4 && k < TRIEXTA_PRODUCTS.length; k++) {
    const p = TRIEXTA_PRODUCTS[(i + k) % TRIEXTA_PRODUCTS.length]
    if (p.handle !== handle) {
...
export default async function TriextaProductPage({ params }) {
  const { handle } = await params
